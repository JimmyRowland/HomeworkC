...
namespace cs20a
{
	...
	public:
		...
		void sort(bool(*comp)(const T &, const T &) = defaultCompare,slist_node<T> *head=head,int size =_size);
	...
	template<typename T>
	void slist<T>::sort(bool(*comp)(const T &, const T &),slist_node<T> *head,int size) {
		if (size <2)
			return;
		else
