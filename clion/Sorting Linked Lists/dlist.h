...
namespace cs20a
{
	...
	public:
		...
		void sort(dlist_node<T> *start, dlist_node<T> *end, bool(*comp)(const T &, const T &) = defaultCompare);
	...
	template<typename T>
	void dlist<T>::sort(dlist_node<T> *start, dlist_node<T> *end, bool(*comp)(const T &, const T &)) {
		if (start == end || head->next == end ||start==nullptr)
			return;
		else {
			if (start == head) start = head->next;
			dlist_node<T> *middle = start;
			for (dlist_node<T> *index = middle->next; index != end->next; index = index->next) {
