...
namespace cs20a
{
	...
	public:
		...
		void sort(slist_node<T> *start,slist_node<T> *end, bool(*comp)(const T &, const T &) = defaultCompare);
	...
	template<typename T>
	void slist<T>::sort(slist_node<T> *start,slist_node<T> *end, bool(*comp)(const T &, const T &) ) {
		if (start==end || head->next==end || start == nullptr)
			return;
		else{
			if(start==head) start=head->next;
			slist_node<T> *middle=start;
			for(slist_node<T> *index=middle->next;index!=end->next;index=index->next){
				if(comp(index->value,start->value)){

					T temp=middle->next->value;
					middle->next->value=index->value;
					index->value=temp;
					middle=middle->next;
				}
			}
			T exchange=start->value;
			start->value=middle->value;
			middle->value=exchange;
			sort(start,middle, comp);
			sort(middle->next,end, comp);
		}
