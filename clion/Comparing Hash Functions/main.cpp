...
int main()
{
    ...
    map<unsigned long, vector<string> > hfKeys;

    ifstream fin;
    char buf[MAX_CHARS_PER_LINE];

    fin.open("/media/toor/storage/vsProject/clion/Comparing Hash Functions/dictionary.txt");
...
//        cout<<djb2Keys[hkey][0]<<endl;

        string word1(buf);
//        cout<<word1<<endl;

        hkey = sdbm((unsigned char *)buf);
        sdbmKeys[hkey].push_back(word1);
//        cout<<sdbmKeys[hkey][0]<<endl;
        string word2(buf);
//        hkey = loselose((unsigned char *)buf);
//        loseloseKeys[hkey].push_back(word2);
        string word3(buf);
        hkey = hf((unsigned char *)buf);
        hfKeys[hkey].push_back(word3);
