...
namespace cs20a
{
    ...
    {   if(!empty()) return closestValue(value,root->value,root);
        else throw std::logic_error("empty root.");
    }
    ...
    {
        T result=closest;
//		cout<<"before"<<closest<<endl;
//		T test=closest;
//		test++;
//		cout<<"after"<<closest<<endl;
        if (node != nullptr){
...
//				closest=root->value;
                result=closest;
            }else if(std::abs(node->value-value)<std::abs(value-closest)){
//				closest = node->value;
                result=node->value;
//				closest = closest(node->value);
            } else{
//				closest = closest;
                result=closest;
            }
            if(value<node->value){
                return closestValue(value,result,node->left);
            }else{
                return closestValue(value,result,node->right);
            }
